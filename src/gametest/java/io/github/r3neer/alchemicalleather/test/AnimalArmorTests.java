...
public final class AnimalArmorTests {
    ...
    @GameTest public void cauldronColorAndWashGeneralizeWithDyeability(GameTestHelper h){
        ...
        for(var item:List.of(Items.IRON_HELMET,Items.COPPER_HORSE_ARMOR)){
            var contents=new PotionContents(item==Items.IRON_HELMET?Potions.NIGHT_VISION:Potions.LUCK);CauldronService.write(h.getLevel(),pos,contents,Items.POTION,1);var input=new ItemStack(item);input.setCustomName(net.minecraft.network.chat.Component.literal("keep me"));
            h.assertTrue(use(h,p,pos,input)==InteractionResult.SUCCESS,"Potion infusion works on synthetic dyeable armor "+item);var infused=p.getMainHandItem();h.assertTrue(infused.get(DataComponents.DYED_COLOR).rgb()==(contents.getColor()&0xffffff),"Potion color applied to "+item);h.assertTrue(infused.getCustomName()!=null&&infused.getCustomName().getString().equals("keep me"),"Unrelated components preserved on "+item);
