...
public final class DyeColors {
    ...
    public static int blend(int... colors) {
        ...
        int peakSum=0,redSum=0,greenSum=0,blueSum=0;
        for(int raw:colors) {
        ...
        int red=redSum/colors.length,green=greenSum/colors.length,blue=blueSum/colors.length;
        int peak=Math.max(red,Math.max(green,blue));
        if(peak==0)return 0;
        float targetPeak=(float)peakSum/colors.length;
